...
% 
load('test_data_synchro/stock_xcenter.mat');
load('test_data_synchro/stock_ycenter.mat');
load('test_data_synchro/stock_bbs.mat');
% 
...
%     end
    ...
    for i=1:length(c)
        center_x{i} = stock_xcenter{pic_cam(i)};
        center_y{i} = stock_ycenter{pic_cam(i)};
        
        bbs_stock{i} = stock_bbs{pic_cam(i)};
...
seuil_dbha_cam = [0.065; 0.065; 0.065];%r(j);
coef_dbha = ones(1,3).*0.2;
for cam=1:ncameras
    cpt = 1; new = 1; % New tracklet is created
    
    seuil_dbha = seuil_dbha_cam(cam);
...
%We take one cam at a time
 for cam = 1:ncameras; 
    ...
    seuil_bbs = [0.2;0.15;0.25];% We accept an error of 'seuil_bbs*100'%
    seuil_dbha = [0.4; 0.3; 0.35];
...
save('test_data_synchro/tracklets.mat', 'tracklets_final');
save('test_data_synchro/tracklets_picnb.mat', 'Pic_final');
save('test_data_synchro/tracklets_framenb.mat', 'Frame_number_final');
save('test_data_synchro/tracklets_ids.mat', 'identity_final');
save('test_data_synchro/tracklets_bbs.mat', 'bbs_final');
save('test_data_synchro/tracklets_histR.mat', 'histR_final');
save('test_data_synchro/tracklets_histG.mat', 'histG_final');
save('test_data_synchro/tracklets_histB.mat', 'histB_final');

...
nb_per_frame_gt = histc(bbs_gt(:,1),ubbs_gt);
 ...
 figure;
cpt=1; cpt_gt=1; l=1925;%after picture cam 1 2 3
seuil = 0.7; % overlap accepted         %seuil = 20; %error tolerance : 'seuil' pixels
...
%                        y_center_gt = (2*bbs_gt(n_gt,2) + bbs_gt(n_gt,4));
                    ...
                    if (overlapRatio > seuil )
                        ...
                        %Stock info bbs
                        stock_xcenter{l-1924} = x_center;
                        stock_ycenter{l-1924} = y_center;
                        stock_bbs{l-1924} = bbs(n,2:8);
                        ...
                        picture{l-1924}     =   imresize(imagette,[H,W], 'bilinear' ); % normalization 64x128
                        imwrite(picture{l-1924}, [rep_imagette 'personne' num2str(bbs_gt(n_gt,6),'%2.2d'), 'detection' num2str(l,'%4.4d')  'frame', num2str(bbs(n,1)+5014, '%3.3d'), 'cam', num2str(bbs(n,7), '%d'), '.jpg']);
                        picture_gt{l-1924} = imresize(imagette_gt,[H,W], 'bilinear' );
...
% %                         pause(0.25);
                        ...
                        if maskon
                            ...
                            mask_fin(:,:,l-1924) = imfill(imresize(m_c,[H,W],'bilinear'),'holes');
